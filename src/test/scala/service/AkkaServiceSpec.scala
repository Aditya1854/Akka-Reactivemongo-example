...
class AkkaServiceSpec extends AnyWordSpec with Matchers with ScalaFutures with ScalatestRouteTest with AkkaHttpDemo with MockitoSugar {
  ...
  override def authenticated: Directive1[Map[String, Any]] = {
    provide(res)
  }

  val student: List[Student] = List(Student(1, "aditya", "aditya@gmail.comm", 1, "1996-05-02"))
  ...
  "Student Routes test all routes of student" should {
    ...
    "create a student" in {
      ...
      Post("/student/create", write(student.head)) ~> route ~> check {
        status should ===(StatusCodes.OK)
        contentType should ===(ContentTypes.`text/plain(UTF-8)`)
        entityAs[String] should ===("""true""")
      }
    ...
    "update a student" in {
      ...
      Put("/student/update", write(student.head)) ~> route ~> check {
        status should ===(StatusCodes.OK)
        contentType should ===(ContentTypes.`text/plain(UTF-8)`)
        entityAs[String] should ===("""true""")
      }
  ...
  "University Routes test all routes of university" should {
    ...
    "create a university" in {
      ...
      Post("/university/create", write(university.head)) ~> route ~> check {
        status should ===(StatusCodes.OK)
        contentType should ===(ContentTypes.`text/plain(UTF-8)`)
        entityAs[String] should ===("""true""")
      }
    ...
    "update a university" in {
      ...
      Put("/university/update", write(university.head)) ~> route ~> check {
        status should ===(StatusCodes.OK)
        contentType should ===(ContentTypes.`text/plain(UTF-8)`)
        entityAs[String] should ===("""true""")
      }
